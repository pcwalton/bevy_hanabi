...
impl<T: Pod + ShaderSize> AlignedBufferVec<T> {
    ...
    #[inline]
    #[allow(dead_code)]
    pub fn range_binding(&'_ self, offset: u32, count: u32) -> Option<BindingResource<'_>> {
        assert!(count > 0);
        let buffer = self.buffer()?;
...
impl HybridAlignedBufferVec {
    ...
    #[allow(dead_code)]
    #[inline]
    pub fn max_binding(&'_ self) -> Option<BindingResource<'_>> {
        // FIXME - Return a Buffer wrapper first, which can be unwrapped, then from that
        // wrapper implement all the xxx_binding() helpers. That avoids a bunch of "if
        // let Some()" everywhere when we know the buffer is valid. The only reason the
        // buffer might not be valid is if it was not created, and in that case
    ...
    #[allow(dead_code)]
    #[inline]
    pub fn lead_binding(&'_ self, size: u32) -> Option<BindingResource<'_>> {
        let buffer = self.buffer()?;
    ...
    #[allow(dead_code)]
    #[inline]
    pub fn range_binding(&'_ self, offset: u32, size: u32) -> Option<BindingResource<'_>> {
